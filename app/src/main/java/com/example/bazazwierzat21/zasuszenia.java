...
public class zasuszenia extends AppCompatActivity {

    @RequiresApi(api = Build.VERSION_CODES.O)
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_zasuszenia);
        EditText result;
        ...
        if(dane_z_servera.length() > 0) {
            ...
            List<Krowa> Lista = Modyfikuj.Zasuszone(ListaCala);
            String wynik = "Zasuszenia\n\n";
            if (Lista.size() > 0) {
